...
def _d_library_cycles_impl(name, deps, implementation_deps, link_only_deps, cycle_breaker_suffix, **kwargs):
    ...
    # to dependencies. We drop `deps`, `implementation_deps`, AND
    # `link_only_deps` here. If any survived, the leaf would form bazel
    # target-graph cycles whenever its containing SCC member's deps closed
    # back on it.
    d_library_break(
